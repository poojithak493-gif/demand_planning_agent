...
class FetchDistributorContextService:
    ...
    def execute(self, distributor_id: str):
        ...
        existing_skus = self.graph_repo.get_existing_skus_for_distributor(
            distributor_id
        )

        sku_demand_map = self._build_sku_demand_map(sales_rows)
        sku_demand_list = self._build_response_list(sku_demand_map)
    ...
    def _get_distributor(self, distributor_id: str):
        distributor = self.distributor_repo.get_by_distributor_id(
            distributor_id
        )

        if not distributor:
            raise ValueError(
                f"Distributor not found: {distributor_id}"
            )

        return distributor

    def _build_sku_demand_map(self, sales_rows):
        sku_demand_map = {}
        ...
        for row in sales_rows:
            ...
            self._update_entry(sku_demand_map[sku_id], row)

        self._calculate_average(sku_demand_map)

        return sku_demand_map

    def _update_entry(self, entry, row):
        entry["total_quantity"] += (
            row.gross_dispatch_value or 0
        )
        entry["order_count"] += 1
        ...
        if txn_date and (
            entry["last_purchase_date"] is None
            or txn_date > entry["last_purchase_date"]
        ):
            entry["last_purchase_date"] = txn_date

    def _calculate_average(self, sku_demand_map):
        for entry in sku_demand_map.values():
            count = entry["order_count"]

            entry["avg_quantity"] = (
                round(entry["total_quantity"] / count, 2)
                if count > 0
                else 0
            )

    def _build_response_list(self, sku_demand_map):
        result = []
        ...
        for entry in sku_demand_map.values():
            ...
            if item["last_purchase_date"]:
                item["last_purchase_date"] = (
                    item["last_purchase_date"].isoformat()
                )
